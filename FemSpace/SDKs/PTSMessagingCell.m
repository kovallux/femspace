...
+(CGSize)messageSize:(NSString*)message {
    ...
    UIFont * font = [UIFont systemFontOfSize:messageTextSize];
...
+(UIImage*)balloonImage:(BOOL)sent isSelected:(BOOL)selected {
    if (sent == YES && selected == YES) {
        return [[UIImage imageNamed:@"balloon_PINK_OUTLINE_right"] stretchableImageWithLeftCapWidth:24 topCapHeight:15]; // selected
    } else if (sent == YES && selected == NO) {
        return [[UIImage imageNamed:@"balloon_PINK_OUTLINE_right"] stretchableImageWithLeftCapWidth:24 topCapHeight:15];
